...
export class Rendertron {
  ...
  async initialize() {
    ...
    this.app.use(
      route.get(
        ...
        this.handlePlayersRequest.bind(this)
      )
    );
    this.app.use(
  ...
  handlePlayersRequest(ctx: Koa.Context, playerHandle: string) {
    const PROJECT_SERVICE_MAP: any = {
      "game-tv-engg": "tournaments-qa",
    ...
    const profileUrl = `https://${targetService}-dot-${currentProject}.appspot.com/user/views/profile/${playerHandle}`;
    console.log("handlerUrl: ", profileUrl);
    return this.handleRenderRequest(ctx, profileUrl);
  }
